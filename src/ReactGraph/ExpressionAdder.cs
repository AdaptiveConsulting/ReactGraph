        ...
        IValueSource<T> CreateSourceNode<T>(ISourceDefinition<T> source, Action<Exception> onError)
        {
            switch (source.NodeType)
            {
                ...
                case NodeType.Member:
                    ...
                    return new ReadOnlyNodeInfo<T>(_ => getValueDelegate(), memberDefinition.FullPath, memberDefinition.PathToParent, nodeRepository, shouldTrackChanges, onError, NodeType.Formula);
                default:
                    throw new ArgumentOutOfRangeException();
            }
        }
